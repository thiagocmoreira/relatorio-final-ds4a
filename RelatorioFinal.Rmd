...
dados.geocapes <- read.csv("dados/dados-geocapes.CSV", sep=";")
aggregated <- aggregate(dados.geocapes$Total.Linha, by=list(dados.geocapes$Ano, dados.geocapes$UF), FUN=sum)
...
ggplot(data=aggregated, aes(x=Group.1, y=x)) + geom_bar(mapping = aes(x=Group.1, fill=Group.2),  stat = "identity") + scale_fill_manual(values = color.list )
...
### 2.4.1 Verificação das palavras e seus sinônimos (Stem)

Descrever...

```{r eval=FALSE}
# titulos <- tm_map(titulos, stemDocument)
```
...
```{r eval=FALSE}
wordcloud2(term_count, color = "random-light", minRotation = -pi/6, maxRotation = -pi/6, minSize = 10, rotateRatio = 1)
...
### 2.6.3 Contagem de palavras chave
...
```{r eval=FALSE}
wordcloud2(term_count, size=1.6, color='random-light', backgroundColor="black")
```

![Departamento de Arquitetura - Nuvem de palavras chave](cloud1.png)

```{r eval=FALSE}
wordcloud2(term_count2, size=1.6)
```
