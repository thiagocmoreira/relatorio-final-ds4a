...
summarized <- dados.geocapes %>% group_by(Ano, UF) %>% summarise(n=n())
...
ggplot(title="Teste",data=summarized, aes(x=Ano, y=n)) + geom_bar( mapping = aes(x=Ano, fill=UF),  stat = "identity") + scale_fill_manual(values = color.list )
